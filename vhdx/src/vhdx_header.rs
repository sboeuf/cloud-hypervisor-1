...
impl Header {
    ...
    pub fn new(f: &mut File, start: u64) -> Result<Header> {
        // Read the whole header in to a buffer. We will need it for calculating checksum
    ...
    /// Converts the header structure in to a buffer
...
impl RegionInfo {
    /// Collects all entries in a BTreeMap from the Region Table and identifies BAT and metadata regions
...
impl RegionTableEntry {
    ...
    pub fn new(buffer: &[u8]) -> Result<RegionTableEntry> {
        ...
        let uuid = Uuid::from_fields_le(
            BigEndian::read_u32(&buffer[0..4]),
            BigEndian::read_u16(&buffer[4..6]),
            BigEndian::read_u16(&buffer[6..8]),
            &buffer[8..16],
        )
        .map_err(VhdxHeaderError::InvalidUuid)?;
...
impl VhdxHeader {
    ...
    /// Identify the current header and return both headers along with an integer indicating the current header
    fn current_header(
    ...
    /// corrent one. Returns both headers as a tuple sequenced as it received
