...
impl Sector {
    /// Translate secotor index and count of data in file to actual offsets and BAT index
    pub fn new(
        disk_spec: &DiskSpec,
        bat: &[BatEntry],
        sector_index: u64,
...
/// VHDx IO read routine: requires relative sector index and count for the desired data
pub fn read(
    f: &mut File,
    buf: &mut [u8],
    disk_spec: &DiskSpec,
...
/// VHDx IO write routine: requires relative sector index and count for the desired data
pub fn write(
    f: &mut File,
    buf: &[u8],
        ...
        if disk_spec.has_parent {
            ...
            match bat_entry & vhdx_bat::BAT_STATE_BIT_MASK {
                ...
                | vhdx_bat::PAYLOAD_BLOCK_ZERO => {
                    ...
                    bat[sector.bat_index as usize] = BatEntry(updated_bat_entry);
