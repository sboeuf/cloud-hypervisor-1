...
impl Vhdx {
    /// Parse the Vhdx header, BAT, and metadata from a file and store info in Vhdx structure
...
impl Read for Vhdx {
    /// Wrapper function to satisfy the Read trait implementation for VHDx disk.
    /// Converts the offest to secotor index and buffer length to sector count.
...
impl Write for Vhdx {
    ...
    /// Wrapper function to satisfy the Write trait implementation for VHDx disk.
    /// Converts the offest to secotor index and buffer length to sector count.
...
impl Seek for Vhdx {
    /// Wrapper function to satisfy the Seek trait implementation for VHDx disk. Updates the offset field in the Vhdx struct.
