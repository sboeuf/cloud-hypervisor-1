...
// Little-endian only; "mixed" would advertise unimplemented big-endian.
const IDR0_TTENDIAN_LE: u32 = 0b10 << 21;
// Termination only; 0b00 would advertise an unimplemented stall/resume model.
const IDR0_STALL_MODEL_NONE: u32 = 0b01 << 24;
const IDR0_STLEVEL_2LVL: u32 = 0b01 << 27; // 2-level Stream Table support
// Stage-1 only: stage-2 belongs to the host, and S2P would advertise nesting
// this emulation does not implement. ATS and SSIDSIZE come from the host.
...
// Set when a record is lost to a full queue; acked via EVENTQ_CONS.OVACKFLG.
const EVENTQ_PROD_OVFLG: u32 = 1 << 31;

// --- Queue base field masks ---
// Q_BASE[4:0] = LOG2SIZE, ADDR is bits [55:5] with [4:0] treated as zero.
...
// Smallest architectural SPLIT; reserved values behave as this.
const STRTAB_SPLIT_MIN: u32 = 6;

const STE_SIZE: u64 = 64;
const STE_WORDS: usize = 8;
const L1STD_SIZE: u64 = 8;
// L1STD.Span: 0 = invalid, else 2^(Span-1) STEs.
...
// Config[2] set => traffic passes; 0b100 is bypass, 0b101/0b110/0b111 translate.
...
/// A decoded Stream Table Entry. `words` keeps the raw STE, which a nesting
/// backend hands to the host as-is.
#[derive(Clone, Copy, Debug)]
...
pub struct Smmuv3 {
    ...
    // Advertised ID registers, refined from the host.
    idr0: u32,
    idr1: u32,
    idr5: u32,
    ...
    // Stored but unused in wired-SPI mode.
    gerror_irq_cfg0: u64,
    eventq_irq_cfg0: u64,
...
impl Smmuv3 {
    ...
    /// Replace the no-op backend, once the endpoints are bound.
    ...
    /// Refine the advertised ID registers from the host's, so the guest never
    /// negotiates a feature the physical IOMMU cannot honor.
    ///
    /// `ats_supported` must agree with the IORT root-complex ATS attribute:
    /// `arm_smmu_ats_supported()` requires both.
    ...
    /// SubstreamID (PASID) bits advertised in IDR1.SSIDSIZE; 0 means no PASID.
    ...
    fn write_reg(&mut self, offset: u64, val: u64, len: usize) {
        match offset {
            ...
            GERROR_IRQ_CFG0 => {
                self.gerror_irq_cfg0 = merge64(self.gerror_irq_cfg0, val, len, false)
            }
            GERROR_IRQ_CFG0_HI => {
            ...
            EVENTQ_IRQ_CFG0 => {
                self.eventq_irq_cfg0 = merge64(self.eventq_irq_cfg0, val, len, false)
            }
            EVENTQ_IRQ_CFG0_HI => {
    ...
    fn consume_cmdq(&mut self) {
        // Gated on CMDQEN alone: Linux enables it first and SMMUEN last, so also
        // requiring SMMUEN would stall its initial CMD_SYNC.
        if self.cr0 & CR0_CMDQEN == 0 {
            return;
        }

        let log2size = (self.cmdq_base & Q_BASE_LOG2SIZE_MASK) as u32;
        // Reject more than the advertised 2^19 entries: shift overflow.
        if log2size > 19 {
        ...
        // Bounded against a malformed PROD.
        for _ in 0..entries {
    ...
    /// Walk the guest stream table and decode the STE for `sid`. `Ok(None)` if
    /// the StreamID is out of range or the entry is invalid.
    fn fetch_ste(&self, sid: u32) -> Result<Option<SteConfig>, GuestMemoryError> {
        ...
        let ste_addr = if fmt == STRTAB_FMT_2LEVEL {
            // StreamID[LOG2SIZE-1:SPLIT] indexes L1, [SPLIT-1:0] indexes L2.
            let split =
    ...
    fn dispatch_command(&mut self, cmd: &Command) {
        let result = match cmd.opcode {
            ...
            CMD_CFGI_STE_RANGE => {
                // STEs are fetched on demand, so there is no cached state.
                debug!(
    ...
    /// Signal CMD_SYNC completion, raising the interrupt if CS == 0b01.
    ...
    /// Tell the guest its event queue missed records, which is all the
    /// architecture lets us say about events lost before they reached us.
    pub fn set_event_overflow(&mut self) {
        self.eventq_prod |= EVENTQ_PROD_OVFLG;
    }

    /// Push an event record, whose StreamID is already the guest's virtual one,
    /// onto the guest's event queue. Dropped and flagged if disabled or full.
    pub fn push_event(&mut self, record: &[u64; 4]) {
        ...
        // Full when the next producer index equals the consumer.
...
/// See [`Smmuv3::set_id_regs_from_host`].
fn refine_id_regs(cur_idr1: u32, host_idr: &[u32; 6], ats_supported: bool) -> (u32, u32, u32) {
    ...
    // Adopt the host's PASID width; zero leaves PASID disabled.
    let idr1 = (cur_idr1 & !IDR1_SSIDSIZE_MASK) | (h1 & IDR1_SSIDSIZE_MASK);

    let oas = (IDR5_VALUE & IDR5_OAS_MASK).min(h5 & IDR5_OAS_MASK);
    // Only granules both support; keep the emulated set if the host reports none.
...
/// Merge a `len`-byte write into a 64-bit register; `high` selects the upper half.
