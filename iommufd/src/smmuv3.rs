...
use iommufd_bindings::iommufd::{
    iommu_hw_info_arm_smmuv3, iommu_hwpt_arm_smmuv3,
    iommu_veventq_flag_IOMMU_VEVENTQ_FLAG_LOST_EVENTS,
...
fn veventq_reader_loop(mut veventq: IommufdVEventQ, device: Weak<Mutex<Smmuv3>>, kill: EventFd) {
    ...
    // Sequence of the last vEVENT seen, to detect gaps.
            ...
            match event.data() {
                ...
                VEVENTQ_TOKEN => match veventq.read_arm_smmuv3_events() {
                    Ok(records) => {
                        ...
                        let mut device = device.lock().unwrap();
                        for (header, evt) in records {
                            // The kernel does not report a loss count. Sequence
                            // numbers are monotonic, so a gap of more than one
                            // means that many vEVENTs were dropped in between.
                            if let Some(prev) = last_sequence {
                                let lost = header.sequence.wrapping_sub(prev).wrapping_sub(1);
                                if lost > 0 {
                                    warn!(
                                        "SMMUv3 vEVENTQ lost {lost} events before sequence {}",
                                        header.sequence
                                    );
                            ...
                            match evt {
                                ...
                                // A tail loss: the header carries the flag and
                                // is not followed by a record.
                                None => {
                                    if header.flags
                                        & iommu_veventq_flag_IOMMU_VEVENTQ_FLAG_LOST_EVENTS
                                        != 0
