...
impl VhdxDiskSync {
    pub fn new(f: File, direct_io: bool) -> Self {
        let vhdx_file = match Vhdx::new(f) {
            Ok(v) => Arc::new(Mutex::new(v)),
            Err(e) => {
                panic!("Failed to open VHDX file: {}\n", e);
            }
        };
        VhdxDiskSync {
            vhdx_file,
...
impl DiskFile for VhdxDiskSync {
    ...
    fn new_async_io(&self, _ring_depth: u32) -> DiskFileResult<Box<dyn AsyncIo>> {
        Ok(Box::new(VhdxSync::new(
            self.vhdx_file.clone(),
...
impl VhdxSync {
    pub fn new(vhdx_file: Arc<Mutex<Vhdx>>, semaphore: Arc<Mutex<()>>) -> Self {
        VhdxSync {
            vhdx_file,
