...
// ACPI `_DSD` device properties describing a passthrough device's coherent
// memory (e.g. an NVIDIA Grace GPU's cacheable aperture), read by the guest
// NVIDIA driver via `nvidia,gpu-mem-*`. `base_pa`/`size` give the coherent
// memory's guest-physical location; `pxm_start`/`pxm_count` the NUMA proximity
...
pub(crate) struct PciSegment {
    ...
    // Per-device-slot coherent-memory `_DSD` (keyed by PCI device number), for
    // passthrough devices whose coherent memory must be described to the guest.
...
impl PciSegment {
    ...
    /// Record the coherent-memory `_DSD` for the device at PCI device number
    /// `slot`, so it is emitted under that device in the DSDT.
...
impl Aml for PciDevSlot {
    fn to_aml_bytes(&self, sink: &mut dyn acpi_tables::AmlSink) {
        ...
        // `Method`/`MethodCall`/`Path` hold references to their arguments, so
        // each must be a named binding that outlives the `Device` below.
        let ej0_sun = aml::Path::new("_SUN");
        let ej0_seg = aml::Path::new("_SEG");
        let ej0_call =
            aml::MethodCall::new("\\_SB_.PHPR.PCEJ".into(), vec![&ej0_sun, &ej0_seg]);
        let ej0 = aml::Method::new("_EJ0".into(), 1, true, vec![&ej0_call]);

        let mut children: Vec<&dyn Aml> = vec![&sun_name, &adr_name, &ej0];

        // Emit a `_DSD` describing coherent GPU memory (NVIDIA Grace), if this
        // slot has one. `PackageBuilder` serializes eagerly, so there are no
        // borrow-lifetime chains between the nested property packages. `Name`
        // owns its serialized bytes, so `dsd` is self-contained once built.
        let dsd;
