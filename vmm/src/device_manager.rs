...
pub enum DeviceManagerError {
    ...
    /// Cannot create an iommufd vIOMMU
    #[cfg(all(target_arch = "aarch64", feature = "kvm"))]
    #[error("Cannot create an iommufd vIOMMU")]
    IommufdViommuCreate(#[source] iommufd_ioctls::IommufdError),

    /// Cannot create an iommufd vDevice
    #[cfg(all(target_arch = "aarch64", feature = "kvm"))]
    #[error("Cannot create an iommufd vDevice")]
    IommufdVdeviceCreate(#[source] iommufd_ioctls::IommufdError),

    /// Cannot attach a VFIO device to the vIOMMU's bypass page table
    #[cfg(all(target_arch = "aarch64", feature = "kvm"))]
...
impl DeviceManager {
    ...
    fn create_nested_vfio_device(
        ...
        let (device, device_path) = match (&device_cfg.path, device_cfg.fd) {
            (Some(path), None) => {
                let cdev = VfioDevice::open_cdev(path).map_err(DeviceManagerError::VfioCreate)?;
                let device = VfioDevice::new_from_fd(cdev, Arc::clone(&vfio_iommufd), false)
                    .map_err(DeviceManagerError::VfioCreate)?;
                (device, path.clone())
            }
            (None, Some(fd)) => {
                ...
                // SAFETY: dup_fd is a freshly-opened fd owned by this File.
                let file = unsafe { File::from_raw_fd(dup_fd) };
                let device = VfioDevice::new_from_fd(file, Arc::clone(&vfio_iommufd), false)
                    .map_err(DeviceManagerError::VfioCreate)?;
        ...
        let dev_id = device
            .iommufd_dev_id()
            .expect("bound to an iommufd by new_from_fd");

        let vdevice = {
            let mut shared = virtual_iommu_fd.shared_viommu().lock().unwrap();
            let viommu = match shared.as_ref() {
                // Devices behind the same guest vIOMMU must share one instance.
                Some(viommu) => Arc::clone(viommu),
                None => {
                    let viommu = Arc::new(
                        IommufdVIommu::new(
                            ...
                            dev_id,
                            virtual_iommu_fd.hw_queue(),
                        )
                        .map_err(DeviceManagerError::IommufdViommuCreate)?,
                    );
                    *shared = Some(Arc::clone(&viommu));
                    viommu
                }
            };

            let vdevice = IommufdVDevice::new(Arc::clone(&viommu), dev_id, virt_id)
                .map_err(DeviceManagerError::IommufdVdeviceCreate)?;

            // Bypass until the guest enables stage-1 translation.
            viommu
                .attach_bypass(device.as_ref())
                .map_err(DeviceManagerError::IommufdAttachBypass)?;
        ...
        if let Some((base_pa, size)) = {
            ...
            dev.bar_by_index(COHERENT_MEM_BAR_INDEX)
                .map(|(base_pa, bar_size)| {
                    // Must be the usable size, not the power-of-two aperture.
                    let size = dev
